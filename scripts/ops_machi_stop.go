...
// machibigbrother: full stop — no trades, no leader stream, no Telegram.
package main
...
func main() {
	...
	token, _ := jwt.NewWithClaims(jwt.SigningMethodHS256, auth.Claims{
		UserID: userID, Email: "machi-stop@local",
	...
	for _, tr := range traders {
		...
		if !strings.Contains(name, "machi") {
			continue
		}
		...
		fmt.Printf("  %-18s running=%-5v paused=%v dry_run=%v layer=%v leader=%s\n",
			tr["trader_name"], tr["is_running"], cc2["copy_paused"], cc2["dry_run"], cc2["copy_layer"], leaderAddr)
	}
}
