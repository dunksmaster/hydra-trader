...
// Max each keeper to wallet liquidity: high notional ceiling + wallet_copy_slots=4
// so size = available/slots * leverage * 0.85 (shrinks as margin is used).
package main
...
func main() {
	...
	token, _ := jwt.NewWithClaims(jwt.SigningMethodHS256, auth.Claims{
		UserID: userID, Email: "max-liquidity@local",
	...
	// Sample equity from first keeper for expected-size printout
	var equity float64
	for _, tr := range traders {
		...
		equity = toF(acct["total_equity"], acct["equity"], acct["available_balance"])
		avail := toF(acct["available_balance"])
		if avail > 0 {
			equity = avail
		}
		break
	...
	slots, lev := 4.0, 10.0
	expected := (equity / slots) * lev * 0.85
	fmt.Printf("equity/avail≈$%.2f → expected notional per open ≈ $%.0f  (avail/%d * %dx * 0.85)\n",
		equity, expected, int(slots), int(lev))
	...
	for _, tr := range traders {
		...
		if cc == nil {
			fmt.Println("  no copy_config")
			continue
		}
		fmt.Printf("  BEFORE notional=%v max_pct=%v slots=%v max_pos=%v\n",
			cc["notional_usd"], cc["max_notional_pct"], cc["wallet_copy_slots"], cc["max_positions"])

		// Ceiling high so wallet_copy_slots liquidity cap always wins.
		cc["notional_usd"] = 1000.0
		cc["min_notional_usd"] = 20.0
		cc["max_notional_pct"] = 0.0 // disable equity% clip that was capping at ~$61
		cc["max_leverage"] = 10
		cc["max_positions"] = 1
		cc["wallet_copy_slots"] = 4 // divide wallet among 4 concurrent legs
	...
	for _, tr := range traders {
		...
		fmt.Printf("%-20s running=%v notional_ceiling=%v max_pct=%v slots=%v max_pos=%v\n",
			name, tr["is_running"], cc["notional_usd"], cc["max_notional_pct"], cc["wallet_copy_slots"], cc["max_positions"])
	}
}
