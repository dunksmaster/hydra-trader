...
import ("encoding/json"; "fmt"; "io"; "net/http"; "os"; "strings"; "nofx/auth")
func main() {
  ...
  if len(latest) == 0 { fmt.Println("no decisions"); return }
  d := latest[0]
  ...
  if len(reasoning) > 2000 { reasoning = reasoning[:2000] + "..." }
