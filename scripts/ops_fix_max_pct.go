...
func main() {
	...
	token, _ := jwt.NewWithClaims(jwt.SigningMethodHS256, auth.Claims{
		UserID: userID, Email: "max-pct-100@local",
	...
	for _, tr := range traders {
		...
		fmt.Printf("%s BEFORE max_pct=%v\n", name, cc["max_notional_pct"])
		...
		fmt.Printf("  AFTER ceiling=%v max_pct=%v slots=%v\n", cc["notional_usd"], cc["max_notional_pct"], cc["wallet_copy_slots"])
	}
	fmt.Println("NOTE: running bots pick up strategy on next fill cycle; restart only if size stays small")
}
