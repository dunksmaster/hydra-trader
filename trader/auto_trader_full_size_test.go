...
func TestApplyAutopilotFullSizeOpenForHyperRank(t *testing.T) {
	...
	// The requested 5x ratio is capped by the book-wide margin budget split
	// across the default 8-position book: 10x leverage * 0.96 budget / 8
	// positions = 1.2x per position, not the raw 5x ceiling.
	if decision.Leverage != 10 || decision.PositionSizeUSD != 56.4 {
		t.Fatalf("hyper_rank Autopilot should full-size within the per-position budget, got lev=%d size=%.2f", decision.Leverage, decision.PositionSizeUSD)
	}
}
